...
<div class="fadeShowGallery">
	<div data-am-fadeshow="next-prev-navigation">
		<!-- Radio -->
		{#each galleryImg as item, i}
			<input type="radio" name="css-fadeshow" id="slide-{i}" />
		{/each}

		<!-- Slides -->
		<div class="fs-slides">
			{#each galleryImg as item, i}
				<div class="fs-slide" style="background-image: url({item});"></div>
			{/each}
		</div>
		...
		<div class="fs-quick-nav">
			{#each galleryImg as item, i}
				<label class="fs-quick-btn" for="slide-{i}"></label>
			{/each}
		</div>
		...
		<div class="fs-prev-nav">
			{#each galleryImg as item, i}
				<label class="fs-prev-btn" for="slide-{i}"></label>
			{/each}
		</div>
		...
		<div class="fs-next-nav">
			{#each galleryImg as item, i}
				<label class="fs-next-btn" for="slide-{i}"></label>
			{/each}
		</div>
	</div>
</div>
	...
	@if $autoplay == true {
		...
		@while $i > 1 {
			...
			@keyframes quickNavAnimation-#{$i} {
				...
				#{100% * calc($fade-time / $total-duration)},
				#{100% * calc($autoplay-duration / $total-duration)} {
					@include quickNavBtnActive();
				}
				#{100% * calc(($autoplay-duration + $fade-time) / $total-duration)},
				100% {
					@include quickNavBtnInactive();
				}
			}
