<script>
    ...
    async function sendMessage() {
        if (newMessage.trim() && !responseWaiting) {
            ...
            messages = [
                ...messages,
                { img: loading_pulse, sender: "bot loading" },
            ];
...
{#if isOpen}
    <div class="chat-window" transition:fly={{ y: 100 }}>
        ...
        <div class="flex">
            ...
            />
            <button on:click={sendMessage} disabled={responseWaiting}>
                {#if responseWaiting}
                    <img src={loading_pulse} alt="response waiting" />
                {:else}
