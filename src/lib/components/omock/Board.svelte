    ...
    socket.onmessage = function (event) {
      ...
      switch (recieved.type) {
        ...
        case "GAME_START":
          ...
          currentUser =
            gameStartData.player1.id === checkId() &&
            gameStartData.player1.unique === checkTs()
...
<div class="flexArea">
  ...
  {#if end}
    <button class="w-btn-neon2" transition:fade on:click={playAgain}>
      Play Again!
    </button>
  {/if}
  ...
  {#each cells as row, i}
    {#each row as _, j}
      <Cell
        ...
        {turn}
        value={_}
        isWinningStone={winningStones.some(
          (stone) => stone.r === i && stone.c === j
        )}
...
<SetIdModal
  show={showSetIdModal}
  {idCookie}
  onSave={setIdAndConnect}
  close={clostSetIdModal}
/>
<OnlineModal
