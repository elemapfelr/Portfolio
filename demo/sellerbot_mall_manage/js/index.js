...
(async () => {
  ...
  document.querySelectorAll("#delLi").forEach((el) => {
    el.addEventListener("click", () => {
      let data_seq_no;
      if (el.parentNode.parentNode.dataset.seq_no) {
        data_seq_no = el.parentNode.parentNode.dataset.seq_no;
      } else {
        data_seq_no = el.parentNode.parentNode.parentNode.dataset.seq_no;
      }
      let userMall = userMallData.find(
        (x) => x.cust_mall_seq_no == data_seq_no
      );
      modalShow(
        `
      <h5 class="mall_cd_nm">${userMall.mall_cd_nm}</h5>
      <span class="mall_cert_1st_id">(${userMall.mall_cert_1st_id})</span>
      <p style="margin-top:10px;">계정을 삭제하시겠습니까?</p>
      `,
        {
      ...
      userMallData.forEach((el) => {
        if (el["cust_mall_seq_no"] == seqNo) {
          ...
          modifyOkBtn.addEventListener("click", () => {
            // 유효성 체크
            let inputArr = modifyBox.querySelectorAll(
              ".main .middle .item input"
            );
  ...
  function addMallCard(data) {
    ...
    registerBox.addEventListener("click", () => {
      ...
      if (window.innerWidth < 860) {
        ...
        const parentTop = document.querySelector(
          "#article_2 .left .contents"
        ).offsetTop;
  ...
  document
    .querySelector("#article_2 #initialize")
    .addEventListener("click", () => {
      if (
        document.querySelector(
          "#article_2 .flexArea .left .contents .modifyArea .modifyBox"
        ) ||
        document.querySelector(
  ...
  const manualBtn = document.querySelector(
    "#article_2 .flexArea .right .topTitle button"
  );
