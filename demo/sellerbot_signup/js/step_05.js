...
(async () => {
	...
	const userMallData = await fetch('./json/userMall.json')
		.then((res) => res.json())
		.then((data) => {
			return data;
		})
	...
	const openedMallData = await fetch('./json/openedMall.json')
		.then((res) => res.json())
		.then((data) => {
			data.forEach((el) => {
				el['count'] = 0;
	...
	const beOpenedMallData = await fetch('./json/beOpenedMall.json')
		.then((res) => res.json())
		.then((data) => {
			data.forEach((el) => {
				el['count'] = 0;
