...
export default function createElement(tag, properties) {
	if (tag == 'toggle') {
		...
		let divRadio = createElement('div', { className: 'radioDiv' });
		divRadio.append(
			createElement('input', {
				type: 'radio',
		...
		if (properties['innerText'] || properties['innerHTML']) {
			divRadio.append(
				createElement('label', {
					className: 'text',
					innerHTML: properties['innerText'] || properties['innerHTML'],
					htmlFor: `${properties['id']}`,
				})
			);
		}
		divRadio.querySelector('input').setAttribute('name', properties['name']);
		return divRadio;
	} else {
